...
**Build update**: 9 of the 14 tiers now use real artwork — wren, panda, badger, fox, pelican, sloth, owl, capybara, flamingo — extracted from uploaded reference images (background/border removed programmatically, recolored to `mountain` ink, transparent PNG). The remaining 5 (tortoise, narwhal, stag, raven, phoenix) still use the simplified single-line placeholder icons described below, since no reference art has been uploaded for them yet.

## Unlock schedule — updated this session

Two new early tiers (days 5, 7) were added to support MVP2 feature-gating (see `ready_MVP2.md`) — every MVP2 feature unlocks alongside a real, earned badge rather than a floating day-count. The original day-1 tier (Curious Otter, "first entry") was dropped — a single day felt too easy to count as an earned trophy, so the ladder now starts at day 3. The original 12 tiers minus that one, plus the 2 new ones, makes this a 14-tier ladder.
...
**Collection screen note**: built as a 3-column grid, 14 slots (4 full rows plus 2 in a fifth) — the grid is data-driven off however many tiers are seeded, so it isn't a fixed 5x3. `ready_prototype.html` itself was never updated to match (it's the original design reference, not the shipped app).
...
- `SpiritTier` table: `id, name, streak_day_threshold, sort_order` — seed with the 14 above.
