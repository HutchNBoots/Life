...
- **[P0, reported from real pilot use] Backfill a missed day's entry via the calendar — currently impossible, not just gated.** A pilot user missed logging a day and had no way to go back and fill it in; today the calendar (dashboard) isn't clickable at all, in any state. Tapping a day should open that day's entry for editing. This sharpens `ready_MVP2.md` Feature 1 (currently gated behind a 3-day streak, editing-only): gating backfill specifically contradicts that same doc's own reasoning — "missed days shouldn't be punishing, and the person most likely to want to open an old day is someone catching up after a gap" — since a brand-new user or anyone who just broke their streak is exactly who's locked out. Recommend reconsidering the gate for backfill specifically (leave it, or gate something else instead) before building Feature 1. Not yet built.
