...
| 5 — Calendar | Done. Current month only, no nav/click-to-edit (both correctly deferred to `ready_MVP2.md` per the doc) |
| 6 — Dashboard | Done. Combined header, journal card (real API, real empty-state copy), streak strip, CTA/banner swap, calendar embed |
| 7 — Settings | Done. Goal label editing, motion/haptics prefs. **No Dev section** — never built, not just hidden |
...
## Not built (by design, all P1/P2 in the backlog, none blocking)

Streak-at-risk indicator, streak milestone celebrations, goal-adherence graph (explicitly optional), backfilled-entry visual distinction, undo-on-save, PIN per profile, >2 goals support, jump-to-date calendar, month-over-month comparison, dark/light mode, PWA manifest.
